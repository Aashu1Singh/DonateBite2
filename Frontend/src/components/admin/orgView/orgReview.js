...
export default function AdminReviewOrg() {
    ...
    swal({
      ...
      if (willDelete) {
        axios
          ...
          .then(() => {
            if (willDelete) {
              swal("The Organization Details Successfully Updated !", {
                icon: "success",
              });
              setTimeout(function () {
  ...
  const onAccept = (id) => {
    ...
    swal({
      ...
      if (willDelete) {
        axios.put(`http://localhost:8070/admin/uporgstatus/${id}`).then(() => {
          if (willDelete) {
            swal(
              "The Organization Register Request Has Been Successfully Approved!",
              { icon: "success" }
            );
            setTimeout(function () {
              navigate(-1);
      ...
      if (willDelete) {
        axios
          ...
          .then(() => {
            if (willDelete) {
              swal(
                "The Organization Register Request Has Been Successfully Deleted!",
                { icon: "success" }
              );
              setTimeout(function () {
                window.location.reload();
